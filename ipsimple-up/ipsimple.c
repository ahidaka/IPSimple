...
char *procfiles[5] = {"/proc/net/ipsm_forward", "/proc/net/ipsm_input",
	"/proc/net/ipsm_output", "/proc/net/ipsm_acct", "/proc/net/ipsm_masquerade"};
...
static char *filename = "/proc/net/ipsm_control";
...
static struct datachain *plast = NULL;
...
do_setsockopt(int cmd, void *data, int length)
{
  ...
  p = write_buffer(cmd, data, length);
  p->next = NULL;
  if (root == NULL) {
    root = plast = p;
  }
...
#if 0 /* #ifdef _DEBUG */
    printf("%d: tot = %d, cmd = %d, len = %d\n",
	   i, total, p->d.cmd, p->d.length);
#endif
    pp = (char *) realloc(pp, total);
    if (ppp == NULL)
      ppp = (struct datas *) pp;
